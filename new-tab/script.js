// Backup: if head redirect is skipped, redirect before paint of interactive UI (same Chrome omnibox limitation as <head> script).
(function () {
    try {
...
// Strip ?nt=1 from the omnibox for a cleaner URL (Chrome still shows chrome-extension://… — that cannot be hidden).
function cleanNewTabUrlForDisplay() {
    try {
        if (new URLSearchParams(window.location.search).get("nt") !== "1") return;
        history.replaceState(null, "", window.location.pathname + window.location.hash);
        sessionStorage.setItem("ntp", "1");
    } catch (_) {}
}
...
// After ?nt=1 navigation, focus + caret on the in-app search field (not needed to fight omnibox like the first new-tab load).
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    initializeSearch();
    focusPageSearchInput();
    window.addEventListener("load", () => focusPageSearchInput());
